...
function SuggestionListPage() {
  ...
  const handleView = (id: string) => {
    if (hasAccess('suggestion', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/suggestions/view/${id}`);
    }
  };
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('suggestion', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/suggestions/create`} passHref legacyBehavior>
                ...
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('document', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/documents/view/${record.document?.id}`}>
